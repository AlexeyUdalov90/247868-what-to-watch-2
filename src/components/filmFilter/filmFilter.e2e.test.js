...
it(`Click on filter item`, () => {
  ...
  const resetFilmList = jest.fn();
  const filmFilter = mount(<FilmFilter genres={[]} activeItem={0} onChangeActiveItem={changeActiveItemHandler} onChangeFilter={changeFilterHandler} resetFilmList={resetFilmList} />);
  ...
  expect(resetFilmList).toHaveBeenCalled();
});
